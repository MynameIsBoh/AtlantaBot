...
class Userinfo extends Command {
    ...
    async run (message, args, data) {
        ...
        let embed = new Discord.MessageEmbed()
            ...
            .addField(message.language.get("USERINFO_FIELDS")[5], message.language.printDate(member.joinedAt),true)
            .addField(message.language.get("USERINFO_FIELDS")[3], user.displayAvatarURL())
            .setColor(data.config.embed.color)
            .setFooter(data.config.embed.footer);

        if(displayPresence){
            embed.addField(message.language.get("USERINFO_FIELDS")[6], message.language.get("UTILS").STATUS[user.presence.status], true);
        }
