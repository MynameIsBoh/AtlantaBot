...
module.exports = class MusicTriviaCommand extends Command {
  ...
  async run(message, { numberOfSongs }) {
    ...
    if (!voiceChannel)return message.say('Please join a voice channel and try again');
    if (message.guild.musicData.isPlaying === true)return message.channel.send('A quiz or a song is already running');
  ...
  static async playQuizSong(queue, message) {
    ...
    queue[0].voiceChannel.join().then(function(connection) {
      const dispatcher = connection
        .play(ytdl(queue[0].url, {quality: 'highestaudio',highWaterMark: 1024 * 1024 * 1024}))
        .on('start', function() {
